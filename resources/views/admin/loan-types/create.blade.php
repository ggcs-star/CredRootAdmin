...
<div class="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
    ...
    <div class="bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-5">
        <h2 class="text-2xl font-bold text-white">Add Loan Type</h2>
        <p class="text-blue-100 text-sm mt-1">Create and manage loan products</p>
    </div>

    <form action="{{ route('loan-types.store') }}" method="POST" enctype="multipart/form-data" class="p-6">
        @csrf
        ...
        <div class="grid grid-cols-1 gap-6">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" placeholder="e.g. Working Capital Loan"
            ...
            <div>
                ...
                <textarea name="description" rows="4" placeholder="Enter loan type description..."
                          class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition">{{ old('description') }}</textarea>
            </div>
        ...
        <div class="flex items-center gap-3 mt-8 pt-6 border-t border-slate-200">
            <button type="submit" class="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-medium shadow-md transition">Save Loan Type</button>
            <a href="{{ route('loan-types.index') }}" class="px-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-medium transition">Back</a>
        </div>
    </form>
</div>
</div>
